...
import { generateAIPlan, generatePTResponse } from '../services/geminiService';
...
const PTDashboard: React.FC<PTDashboardProps> = ({ user, t, clients, requests, plans, messages, onAcceptClient, onRejectClient, onCreatePlan, onUpdateProfile, onUpdateClient, onSendMessage }) => {
  ...
  const handleAiGeneratePlan = async () => {
    ...
    const aiPlan = await generateAIPlan(selectedClientData);
    if (aiPlan) {
        setPlanForm({
            title: `${selectedClientData.name}'s Transformation`,
            workoutDay: aiPlan.workoutDay,
            workoutTitle: aiPlan.workoutTitle,
            exercises: aiPlan.exercises,
            dietTitle: aiPlan.dietTitle,
            dietFocus: aiPlan.dietFocus,
            meals: aiPlan.meals
        });
    } else {
        alert("Could not generate plan. Please try again or check API Key.");
    }
  ...
  const handleAiSuggestReply = async () => {
    ...
    setIsAiLoading(true);
    const lastMsg = activeConversation.length > 0 ? activeConversation[activeConversation.length - 1].body : "Hello";
    const suggestion = await generatePTResponse(activeConversation, user.name, lastMsg);
                        ...
                        <div className="p-4 bg-slate-900 border-t border-slate-700">
                             ...
                             <div className="mb-2 flex justify-end">
                                <button 
                                    ...
                                    className="text-xs text-brand-highlight flex items-center gap-1 hover:text-white transition-colors"
                                >
                                    {isAiLoading ? "Thinking..." : "✨ AI Suggest Reply"}
                                </button>
                             </div>
         ...
         <div className="space-y-6">
            ...
            <div className="flex justify-between items-start">
                ...
                <button 
                    ...
                    disabled={isGeneratingPlan || !selectedClientData}
                    className="flex flex-col items-center justify-center p-2 bg-gradient-to-br from-brand-accent to-brand-highlight text-white rounded-lg text-xs font-bold hover:scale-105 transition-transform disabled:opacity-50"
                >
                    <span className="text-lg">✨</span>
                    <span>{isGeneratingPlan ? "Thinking..." : "AI Auto-Gen"}</span>
                </button>
            </div>
